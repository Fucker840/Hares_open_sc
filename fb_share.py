#CODED BY : MD SABBIR HOSSAIN.RAFI
...
{UT}| $$  | $$| $$  | $$| $$  \ $$| $$\  $$ 
{HT}| $$$$$$$/| $$  | $$| $$  | $$| $$ \  $$
{UT}|_______/ |__/  |__/|__/  |__/|__/  \__/

\033[1;92m WELCOME TO  MD SABBIR HOSSAIN RAFI's TOOLS💞💥…   '''
 cetak(nel(banner,title=f'{P2} {H2}[ {P2}❤️🧡💛💚 {H2}]',subtitle_align='left',padding=1,style='blue'))
	
###----------[ MENU LOGIN ]----------###	
def login():
	os.system("clear")
	cetak(nel(f'   {H2}        LOGIN COOKIES FIRST BRO\n\n      {H2}       [ GITHUB : DARK-404 ]',title=f'{P2} {H2}[ {OT} WELCOME SIR💥 {H2}]',width=54,padding=(1,4),style='blue'))
	cetak(nel(f'{P2}        {H2}   TAKE COOKIES FROM KIWI BROWSER',subtitle=f'{PT}â”Œâ”€[ {OT} COOKIES {PT}]',subtitle_align='left',width=54,padding=1,style='blue'))
	cookie = input(f"{P}   😘😘😘> : {H}")
	try:
...
{P2} CURRENT DATE    : {hari}, {tanggal}''',title=f'{P2} {H2}[ {P2}USER INFORMATION {H2}]',subtitle_align='left',padding=1,style='blue'))
	...
	cetak(nel(f'{P2} ENTER THE POST LINK',subtitle=f'{P2}â”Œâ”€',subtitle_align='left',width=25,padding=0,style='blue'))
	link = input(f"{P}   Don't forget rafi💥> : {H}")
	cetak(nel(f'{P2} AMOUNT OF SHARES',subtitle=f'{P2}â”Œâ”€',subtitle_align='left',width=22,padding=0,style='blue'))
	jumlah = int(input(f"{P}   RAFI IS BRAND BRO💥> : {H}"))
	cetak(nel(f'{P2} AUTO SHARE IS RUNNING',subtitle=f'{P2}Sharing is Started',subtitle_align='left',width=29,padding=0,style='blue'))
	...
	try:
		...
		for x in range(jumlah):
			...
			if "id" in post:
				...
				print(f'{P}\r   START💞> {bas} SUCCESSFUL SHARING {H}{n}{P} POST {N} ',end='');sys.stdout.flush()
			else:
				print("\n")
